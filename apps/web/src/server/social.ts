...
export async function toggleFollow(followerId: string, handle: string) {
  const target = await queryOne<{ id: string }>("select id from profiles where handle = $1", [handle]);
...
async function toggleJoin(table: "likes" | "saves", profileId: string, journeyId: string) {
...
export const toggleLike = (profileId: string, journeyId: string) =>
  toggleJoin("likes", profileId, journeyId);

export const toggleSave = (profileId: string, journeyId: string) =>
  toggleJoin("saves", profileId, journeyId);

export async function listComments(journeyId: string) {
  return query(
...
export async function addComment(authorId: string, journeyId: string, body: string) {
  const journey = await queryOne("select 1 from journeys where id = $1", [journeyId]);
  if (!journey) throw new AppError("not_found", "No such journey");
...
export async function deleteComment(commentId: string, requesterId: string) {
  const comment = await queryOne<{ author_id: string; journey_author: string }>(
...
export async function listFollowers(handle: string, direction: "followers" | "following") {
  const join =
    direction === "followers"
