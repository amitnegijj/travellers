...
import { Avatar, Badge, Card, Photo } from "@/components/ui";
import type { JourneyCard as JourneyCardData } from "@/server/journeys";
import { dayCount, formatDistance, formatDuration, formatMoney, relativeTime } from "@/lib/utils";
...
export function JourneyCard({ journey }: { journey: JourneyCardData }) {
  ...
  return (
    <Card className="group overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[var(--shadow-lg)]">
      <Link href={`/journeys/${journey.id}`} className="relative block">
      ...
      <div className="p-3.5">
        ...
        <div className="flex items-center justify-between gap-2 border-t border-[var(--border)] pt-3">
          ...
          <div className="flex shrink-0 items-center">
            ...
            />
            <Link
              href={`/journeys/${journey.id}#comments`}
...
export function JourneyHero({ journey }: { journey: JourneyCardData }) {
  const days = dayCount(journey.startDate, journey.endDate);
  const cost = Number(journey.totalExpenseMinor);
...
export function JourneyRailCard({ journey }: { journey: JourneyCardData }) {
  const days = dayCount(journey.startDate, journey.endDate);
  const cost = Number(journey.totalExpenseMinor);
...
export function JourneyCardSkeleton({ className }: { className?: string }) {
  return (
