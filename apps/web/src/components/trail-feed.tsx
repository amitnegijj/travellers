...
import { cn, formatDistance, formatDuration, formatMoney, relativeTime } from "@/lib/utils";
import type { Trail } from "@/server/journeys";

export function TrailFeed({ trails }: { trails: Trail[] }) {
  const [activeId, setActiveId] = useState<string | null>(trails[0]?.id ?? null);
      ...
      (entries) => {
        for (const e of entries) {
          if (e.isIntersecting && e.intersectionRatio > 0.6) {
            setActiveId((e.target as HTMLElement).dataset.trailId ?? null);
          }
        }
      },
      { root, threshold: [0.6] }
...
function TrailCard({
  ...
  trail: Trail; active: boolean; first: boolean;
}) {
  ...
  const go = useCallback(
    (delta: number) => setI((v) => Math.min(frames, Math.max(0, v + delta))),
    [frames]
  );
  ...
  const toggleLike = useCallback(async () => {
    ...
    const res = await fetch(`/api/v1/journeys/${trail.id}/like`, { method: "POST" });
    if (res.status === 401) {
      setLiked(prev.liked);
  ...
  }, [liked, likeCount, trail.id, router]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-28 right-3 z-20 flex flex-col items-center gap-4 sm:bottom-32 sm:right-5">
        <Link
          href={`/profile/${trail.authorHandle}`}
          className="mb-1"
...
function MicroStat({ icon, value }: { icon: React.ReactNode; value: string | null }) {
...
function RailButton({
  icon, label, count, active, onClick, href, activeClass,
}: {
...
function SaveRail({ trail }: { trail: Trail }) {
  const router = useRouter();
  ...
  const toggle = async () => {
    ...
    const res = await fetch(`/api/v1/journeys/${trail.id}/save`, { method: "POST" });
    if (res.status === 401) {
      setSaved(prev.saved);
      setCount(prev.count);
      router.push("/login");
