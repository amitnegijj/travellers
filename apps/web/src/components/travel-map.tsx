...
const MapCanvas = dynamic(
  ...
  { ssr: false, loading: () => <Skeleton className="h-[440px] w-full rounded-[var(--radius-xl)]" /> }
);
...
type JourneyPlace = {
  id: string; name: string; slug: string; lng: number; lat: number; journeyCount: number;
};
...
export function TravelMap({
  ...
  const markers = useMemo<MapMarker[]>(() => {
    const out: MapMarker[] = journeyPlaces.map((d) => ({
      id: `j-${d.id}`,
      lng: Number(d.lng),
      lat: Number(d.lat),
      label: d.name,
  ...
  async function save() {
    ...
    setPending(true);
    try {
      const res = await fetch(`/api/v1/profiles/${handle}/places`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        setError(data?.error?.message ?? "Could not save that pin");
        return;
      }
      const created: UserPlace = await res.json();
  ...
  async function toggleVisibility(place: UserPlace) {
    const next = place.visibility === "public" ? "private" : "public";
    const previous = places;
    setPlaces((prev) =>
      prev.map((p) => (p.id === place.id ? { ...p, visibility: next } : p))
    );

    const res = await fetch(`/api/v1/user-places/${place.id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ visibility: next }),
    });
    if (!res.ok) setPlaces(previous);
  }

  async function remove(place: UserPlace) {
    const previous = places;
    setPlaces((prev) => prev.filter((p) => p.id !== place.id));
    const res = await fetch(`/api/v1/user-places/${place.id}`, { method: "DELETE" });
    if (!res.ok) setPlaces(previous);
  }
