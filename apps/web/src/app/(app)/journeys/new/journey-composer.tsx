...
type StopDraft = { key: string; name: string; note: string; destinationId: string; arrivedOn: string };
type ExpenseDraft = { key: string; category: string; label: string; rupees: string; spentOn: string };
type TipDraft = { key: string; kind: "tip" | "warning"; body: string };
...
export function JourneyComposer({
  ...
  function buildPayload(publish: boolean) {
    return {
      ...
      difficulty: (difficulty || null) as "easy" | "moderate" | "hard" | null,
  ...
  async function submit(publish: boolean) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data?.error?.message ?? "Could not save your journey");
        return;
      }
              ...
              <ul className="space-y-2.5">
                {tips.map((tip, i) => (
                  <li key={tip.key} className="flex gap-2">
                    <Select
                      value={tip.kind}
                      onChange={(e) => setTips((s) => s.map((x) => x.key === tip.key ? { ...x, kind: e.target.value as "tip" | "warning" } : x))}
