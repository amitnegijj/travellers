...
const MapCanvas = dynamicImport(
  ...
  { loading: () => <Skeleton className="h-[320px] w-full rounded-[var(--radius-xl)]" /> }
);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return { title: d.name, description: d.description ?? undefined };
  } catch {
...
export default async function DestinationPage({ params }: Props) {
  ...
  const places = destination.places as {
    id: string; name: string; description: string | null; address: string | null;
    priceMinor: string | null; currency: string; categoryName: string | null;
    photoUrl: string | null; lng: number; lat: number;
  }[];
  ...
  return (
    <article className="space-y-8">
      ...
      <section>
        ...
        {journeys.length === 0 ? (
          <EmptyState
            ...
            action={
              <LinkButton href={`/journeys/new?destination=${destination.slug}`} variant="create">
                Log a journey
              </LinkButton>
            }
          />
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-[var(--radius-xl)] border border-[var(--border)] bg-gradient-to-br from-[var(--brand-soft)] to-[var(--ai-soft)] p-6">
        ...
        <LinkButton href={`/journeys/new?destination=${destination.slug}`} variant="create" size="lg">
          Share your journey
        </LinkButton>
      </div>
    </article>
