...
export function JourneyDetailPage() {
  ...
  const { data: journey, loading, error } = useApi(
    (signal) => api.get(`/api/v1/journeys/${id}`, signal),
    [id]
  );
  const [followState, setFollowState] = useState(null);

  useDocumentTitle(journey?.title ?? "Journey");

  if (error) {
    return <NotFoundBlock title="This page doesn't exist" description="That journey isn't here — it may have been removed or unpublished." />;
  }
  ...
  const media = journey.media;
  const comments = journey.comments;
  const authorFollow = followState ?? journey.authorFollow;
  ...
  const mapBlock = (
    <Suspense fallback={<Skeleton className="h-[380px] w-full rounded-[var(--radius-xl)]" />}>
      <MapCanvas
        height={380}
  ...
  return (
    <article className="mx-auto max-w-3xl">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {user?.handle === journey.authorHandle ? (
            <div className="ml-auto flex items-center gap-2">
              ...
              <LinkButton to={`/journeys/${journey.id}/edit`} variant="secondary" size="sm">
      ...
      <div className="mt-10 border-t border-[var(--border)] pt-8">
        <CommentSection journeyId={journey.id} initialComments={comments} currentUser={user} />
      </div>
    </article>
  );
}
