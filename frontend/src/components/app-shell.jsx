...
import { Avatar, buttonClass } from "./ui.jsx";
...
export function AppShell({ rail, children }) {
    ...
    <div className="min-h-dvh bg-[var(--bg)]">
      ...
      <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--surface)]/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1600px] items-center gap-4 px-4 lg:px-6">
          ...
          <form onSubmit={submitSearch} role="search" className="mx-auto hidden w-full max-w-lg md:block">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {user ? (
                  ...
                  {menuOpen ? (
                    <>
                      ...
                      >
                        ...
                        <Link to={`/profile/${user.handle}`} role="menuitem" className={menuItem}>
                          <User size={15} /> My profile
                        </Link>
                        <Link to="/journeys/mine" role="menuitem" className={menuItem}>
                          <Luggage size={15} /> My trips &amp; drafts
                        </Link>
                        <Link to="/settings" role="menuitem" className={menuItem}>
                          <Settings size={15} /> Edit profile
                        </Link>
                        <button onClick={handleLogout} role="menuitem" className={cn(menuItem, "w-full text-[var(--danger)]")}>
              ...
              <>
                ...
                <Link to="/signup" className={buttonClass("primary", "sm")}>Join free</Link>
              </>
            )}
          </div>
        </div>
      ...
      <div className="mx-auto flex max-w-[1600px] gap-6 px-4 lg:px-6">
        ...
        <aside className="sticky top-16 hidden h-[calc(100dvh-4rem)] shrink-0 overflow-y-auto py-5 lg:block"
          ...
          {user ? (
            <Link to="/journeys/new" className={cn(buttonClass("create", "md"), "mb-5 w-full")}>
              <Plus size={17} strokeWidth={2.6} /> Create
            </Link>
          ) : (
            <Link to="/signup" className={cn(buttonClass("primary", "md"), "mb-5 w-full")}>
              Join Travelora
            </Link>
          )}
